...
import static org.cbs.actions.CommonActions.*;
import static org.cbs.actions.drivers.DriverActions.withDriver;
import static org.cbs.enums.ListenerType.ELEMENT_ACTION;
...
public class ElementActions implements IElementActions {
    ...
    public boolean isDisplayedWithoutWait () {
        LOGGER.traceEntry ();
        LOGGER.info ("Checking if element located by: {} is displayed without wait", this.locator.getName ());
        ofNullable (this.listener).ifPresent (l -> l.onIsDisplayed (this.locator));
        return LOGGER.traceExit (getElementAttributeWithoutWait(WebElement::isDisplayed,this.locator,false));
    }
