package org.cbs.utils;

import static org.cbs.enums.Message.CONFIG_KEY_NOT_FOUND;
import static org.cbs.utils.JsonUtil.fromFile;
...
import org.cbs.config.FrameworkSetting;
    ...
    public static FrameworkSetting loadSetting () {
        ...
        if (isNull (frameworkSetting)) {
            final var defaultPath = Path.of (getProperty ("user.dir"), "src/test/resources")
                .toString ();
            final var configDirectory = ofNullable (getenv ("CBS_CONFIG_PATH")).orElse (
                ofNullable (getProperty ("cbs.config.path")).orElse (defaultPath));
            final var configPath = Path.of (configDirectory, "cbs-config.json")
                .toString ();
